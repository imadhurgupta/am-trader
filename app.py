...
def delivery_partner_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session or not session.get('is_delivery_partner'):
            flash('Access denied. Delivery Partner privileges required.', 'error')
            return redirect(url_for('login'))
        return f(*args, **kwargs)
    return decorated_function
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            session['is_delivery_partner'] = bool(user.get('is_delivery_partner', 0) if 'is_delivery_partner' in user.keys() else 0)
...
def handle_google_user_login(email, name):
    ...
    if not user:
        ...
        db.execute(
            "INSERT INTO users (full_name, email, password_hash) VALUES (?, ?, ?)",
            (name, email, hash_password(random_pwd))
        )
        db.commit()
        user = db.execute("SELECT * FROM users WHERE email = ?", (email,)).fetchone()
...
@app.route('/delivery-partner', methods=['GET', 'POST'])
def delivery_partner():
    if request.method == 'POST':
        ...
        if not full_name or not email or not phone or not city or not vehicle_type:
            flash('Please fill out all required fields.', 'error')
            return render_template('delivery_partner.html')
        ...
        db.execute(
            """
            INSERT INTO delivery_partner_applications (full_name, email, phone, city, vehicle_type, experience_years, message) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (full_name, email, phone, city, vehicle_type, exp_years, message)
        )
        db.commit()
        db.close()

        flash('Your application has been submitted successfully! The admin team will review it shortly.', 'success')
        return redirect(url_for('delivery_partner'))
...
def delivery_dashboard():
    ...
    db.close()
    return render_template('delivery_dashboard.html', orders=orders_list)


@app.route('/api/delivery/update-status/<int:order_id>', methods=['POST'])
@delivery_partner_required
def delivery_update_status(order_id):
    ...
    valid_statuses = ['Picked Up', 'In Transit', 'Delivered', 'Cancelled']
    ...
    # Map delivery status to admin-visible status
    admin_status_map = {
        'Picked Up': 'Shipped',
        'In Transit': 'Shipped',
        'Delivered': 'Delivered',
    ...
    admin_status = admin_status_map.get(status, status)
    db.execute("UPDATE orders SET status = ? WHERE id = ?", (admin_status, order_id))
    db.commit()
    db.close()

    if request.is_json:
        return jsonify({'success': True, 'status': status, 'admin_status': admin_status})
    flash(f'Order status updated to "{status}".', 'success')
    return redirect(url_for('delivery_dashboard'))


@app.route('/api/delivery/scan-order', methods=['POST'])
@delivery_partner_required
def delivery_scan_order():
    """Mark order as Delivered when scanned by the delivery partner."""
    partner_id = session['user_id']
    if request.is_json:
    ...
    if not order:
        db.close()
        return jsonify({'success': False, 'error': f'Order #{order_id} not found or not assigned to you'}), 404

    if order['status'] == 'Delivered':
        db.close()
        return jsonify({'success': False, 'error': f'Order #{order_id} is already marked as Delivered'}), 409

    db.execute("UPDATE orders SET status = 'Delivered' WHERE id = ?", (order_id,))
    db.commit()
    db.close()

    return jsonify({'success': True, 'message': f'Order #{order_id} marked as Delivered!', 'order_id': order_id})
...
def admin_dashboard():
    ...
    for order in orders_raw:
        ...
        # Fetch assigned delivery partner name
        dp_name = None
        if o_dict.get('delivery_partner_id'):
            dp = db.execute("SELECT full_name FROM users WHERE id = ?", (o_dict['delivery_partner_id'],)).fetchone()
            if dp:
                dp_name = dp['full_name']
        o_dict['delivery_partner_name'] = dp_name
    ...
    # Delivery Partner extensions
    delivery_applications = db.execute("SELECT * FROM delivery_partner_applications ORDER BY created_at DESC").fetchall()
    delivery_partners = db.execute("SELECT * FROM users WHERE is_delivery_partner = 1 ORDER BY full_name").fetchall()
...
@app.route('/admin/orders/<int:order_id>/assign-delivery', methods=['POST'])
@admin_required
def admin_assign_delivery(order_id):
    if request.is_json:
        partner_id = request.json.get('delivery_partner_id')
    else:
    ...
    if partner_id:
        try:
            ...
            partner = db.execute("SELECT * FROM users WHERE id = ? AND is_delivery_partner = 1", (partner_id,)).fetchone()
            if not partner:
                db.close()
                if request.is_json:
                    return jsonify({'success': False, 'error': 'Invalid delivery partner selected'}), 400
                flash("Invalid delivery partner selected.", "error")
                return redirect(url_for('admin_dashboard'))
        except ValueError:
            partner_id = None
    else:
    ...
    db.execute("UPDATE orders SET delivery_partner_id = ? WHERE id = ?", (partner_id, order_id))
    db.commit()
    db.close()
...
@app.route('/admin/approve-delivery-partner/<int:id>', methods=['POST'])
@admin_required
def admin_approve_delivery_partner(id):
    db = get_db()
    app = db.execute("SELECT * FROM delivery_partner_applications WHERE id = ?", (id,)).fetchone()
    if not app:
        db.close()
        flash("Application not found.", "error")
        return redirect(url_for('admin_dashboard'))

    user = db.execute("SELECT * FROM users WHERE email = ?", (app['email'],)).fetchone()
    if user:
        db.execute("UPDATE users SET is_delivery_partner = 1 WHERE id = ?", (user['id'],))
    else:
        random_pwd = secrets.token_hex(16)
        db.execute(
            "INSERT INTO users (full_name, email, password_hash, is_delivery_partner) VALUES (?, ?, ?, 1)",
            (app['full_name'], app['email'], hash_password(random_pwd))
        )
    
    db.execute("UPDATE delivery_partner_applications SET status = 'Approved' WHERE id = ?", (id,))
    db.commit()
    db.close()

    flash(f"Delivery partner application for {app['full_name']} approved successfully!", "success")
    return redirect(url_for('admin_dashboard'))


@app.route('/admin/reject-delivery-partner/<int:id>', methods=['POST'])
@admin_required
def admin_reject_delivery_partner(id):
    db = get_db()
    db.execute("UPDATE delivery_partner_applications SET status = 'Rejected' WHERE id = ?", (id,))
    db.commit()
    db.close()
    flash("Delivery partner application rejected.", "success")
    return redirect(url_for('admin_dashboard'))


@app.route('/admin/delete-delivery-application/<int:id>', methods=['POST'])
@admin_required
def admin_delete_delivery_application(id):
    db = get_db()
    db.execute("DELETE FROM delivery_partner_applications WHERE id = ?", (id,))
    db.commit()
    db.close()
...
@app.route('/admin/revoke-delivery-partner/<int:id>', methods=['POST'])
@admin_required
def admin_revoke_delivery_partner(id):
    db = get_db()
    db.execute("UPDATE users SET is_delivery_partner = 0 WHERE id = ?", (id,))
    db.commit()
    db.close()
    flash("Delivery partner status revoked successfully.", "success")
    return redirect(url_for('admin_dashboard'))


@app.route('/admin/delivery-application/<int:app_id>/action', methods=['POST'])
@admin_required
def admin_delivery_application_action(app_id):
    """Unified approve/reject endpoint for delivery partner applications."""
    action = request.form.get('action', 'reject')
    db = get_db()
    application = db.execute(
        "SELECT * FROM delivery_partner_applications WHERE id = ?", (app_id,)
    ).fetchone()
    if not application:
        db.close()
        flash("Application not found.", "error")
    ...
    if action == 'approve':
        user = db.execute("SELECT * FROM users WHERE email = ?", (application['email'],)).fetchone()
        if user:
            db.execute("UPDATE users SET is_delivery_partner = 1 WHERE id = ?", (user['id'],))
        else:
            random_pwd = secrets.token_hex(16)
            db.execute(
                "INSERT INTO users (full_name, email, password_hash, is_delivery_partner) VALUES (?, ?, ?, 1)",
                (application['full_name'], application['email'], hash_password(random_pwd))
            )
        db.execute("UPDATE delivery_partner_applications SET status = 'Approved' WHERE id = ?", (app_id,))
        db.commit()
        db.close()
        flash(f"✅ Delivery partner application for {application['full_name']} approved!", "success")
    else:
        db.execute("UPDATE delivery_partner_applications SET status = 'Rejected' WHERE id = ?", (app_id,))
        db.commit()
        db.close()
        flash(f"Application for {application['full_name']} rejected.", "success")

    return redirect(url_for('admin_dashboard'))


@app.route('/admin/assign-delivery-partner/<int:order_id>', methods=['POST'])
@admin_required
def admin_assign_delivery_partner_to_order(order_id):
    """AJAX route – assign a delivery partner to a specific order."""
    if request.is_json:
    ...
    if not partner_id:
        return jsonify({'success': False, 'error': 'No partner selected'}), 400
    ...
    db = get_db()
    partner = db.execute(
        "SELECT * FROM users WHERE id = ? AND is_delivery_partner = 1", (partner_id,)
    ).fetchone()
    if not partner:
        db.close()
        return jsonify({'success': False, 'error': 'Delivery partner not found'}), 404

    db.execute("UPDATE orders SET delivery_partner_id = ? WHERE id = ?", (partner_id, order_id))
    db.commit()
    db.close()

    return jsonify({'success': True, 'partner_name': partner['full_name']})
