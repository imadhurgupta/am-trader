...
def init_db():
    ...
    # Safe migration for delivery partner columns
    try:
        cursor.execute("ALTER TABLE users ADD COLUMN is_delivery_partner INTEGER DEFAULT 0")
        conn.commit()
    except sqlite3.OperationalError:
        pass

    try:
        cursor.execute("ALTER TABLE orders ADD COLUMN delivery_partner_id INTEGER")
        conn.commit()
    except sqlite3.OperationalError:
        pass
    ...
    if admin_count == 0:
        admin_email = 'admin@thesaveur.in'
        ...
        cursor.execute(
            ...
            ('System Admin', admin_email, admin_pass_hash)
        )
        conn.commit()
        print("[OK] Seeded default admin account (admin@thesaveur.in / admin123).")
